...
class Genome():
    def __init__(self, fasta_filename, map_filename):
        ...
        self._map_handles = [
            pysam.TabixFile(map_filename + '_%d.gz' % r)
    ...
    def get_mappability(self, transcripts):
        ...
        for transcript in transcripts:
            ...
            mappables = [
                np.zeros(transcript.mask.shape, dtype='bool')
                for r in utils.READ_LENGTHS
            ]
            tbx_iters = [
                handle.fetch(transcript.chromosome, transcript.start,
                             transcript.stop) for handle in self._map_handles
            ]
            if transcript.strand == '+':
                offsets = [1, 1, 1, 1]
            else:
            ...
            for tbx_iter, mappable, offset in zip(tbx_iters, mappables,
                                                  offsets):
...
class RiboSeq():
    def __init__(self, file_prefix):
        ...
        self._fwd_handles = [
            pysam.TabixFile(file_prefix + '_fwd.%d.gz' % r)
            for r in utils.READ_LENGTHS
        ]
        self._rev_handles = [
            pysam.TabixFile(file_prefix + '_rev.%d.gz' % r)
            for r in utils.READ_LENGTHS
        ]
    ...
    def get_counts(self, transcripts):
        ...
        for transcript in transcripts:
            ...
            rcounts = [
                np.zeros(transcript.mask.shape, dtype='int')
                for r in utils.READ_LENGTHS
            ]
            if transcript.strand == '+':
...
class RnaSeq():
    def __init__(self, filename):
        ...
        for chrom in self._handle.contigs:
            self.total += reduce(lambda x, y: x + y,
                                 (int(tbx.split('\t')[3])
                                  for tbx in self._handle.fetch(chrom)))
    ...
    def get_total_counts(self, transcripts):
        ...
        for transcript in transcripts:
            ...
            tbx_iter = self._handle.fetch(transcript.chromosome,
            ...
            total_counts.append(
                max([1, counts]) * 1e6 / float(transcript.L * self.total))
...
def load_gtf(filename):
    ...
    for line in handle:
        ...
        attr = dict([(ln.split()[0], eval(ln.split()[1]))

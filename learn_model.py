...
def parse_args():
    parser = argparse.ArgumentParser(
        description=" learns the parameters of riboHMM to infer translation "
    ...
    parser.add_argument(
        ...
        default=1e-4,
        help=
        "convergence criterion for change in per-base marginal likelihood (default: 1e-4)"
    )
    ...
    parser.add_argument(
        ...
        default=1000,
        help=
        "number of transcripts used for learning model parameters (default: 1000)"
    )
...
def learn(options):
    ...
    for i, r in enumerate(utils.READ_LENGTHS):
        print("%d ribosome footprints of length %d bp" % (np.sum(
            [c[:, i].sum() for c in footprint_counts]), r))
    ...
    for i, r in enumerate(utils.READ_LENGTHS):
        print("%d bases have missing counts for %d bp footprints" % (np.sum(
            [m.shape[0] - np.sum(m[:, i]) for m in rna_mappability]), r))
